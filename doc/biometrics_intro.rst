...
By using biometrics it is possible to establish and confirm an identity based on *who you are* instead of *what you possess* like a key or ID-card, or *what you remember* like a password.
...
In some application, biometrics may be used to supplement an ID-card or a password, adding another level of security (dual-factor authentication).
...
Physical attributes:
  ...
  - **Fingerprints patterns**: This attribute has been used for a long time and has a very high accuracy. Fingerprints are different between twins.
  - **Voice**: This trait is also commonly used by humans. It can either be a physical or behavioral attribute.
  - **Vein patterns**: Veins follow a path that is unique to individuals, but are not or partly visible without specialized sensors.
...
Chemical attributes:
  - **DNA**: Although difficult and costly to analyze, the DNA is unique to every individual.
...
A biometric system typically operates on a series of subsystems going from the capture of a sample with a sensor, to a feature extraction, to finally either register a subject, or estimate the identity of a subject.


Acquisition of the sample
-------------------------

A first rather important step is the acquisition of a sample of the biometric attribute we want to measure. This is done by a sensor.

The type of sensor used will depend on the biometric attribute being captured, it could be a RGB camera (for face recognition), a microphone (voice recognition), or a sensor designed specifically for the task (fingerprint or iris scanners).
...
In order to remove variability on the samples coming from the sensor, a preprocessing step is often present. This operation can normalize a sample (balance contrast and luminosity of an image), or remove unnecessary data (background of an image, silence of a voice recording)
...
This is done by a transformer (also called extractor).
...
Since there is a lot of variability in biometrics (ageing, make-up, clothing, sensor noise), two samples of the same person are not likely to have the exact same feature vector.
Thus, a comparison between the vectors is needed and results in a score that should shows how much the samples belong to the same person.
...
.. figure:: img/comparison_score.png
  ...
  Verification: Two sample extracted and compared giving a similarity score
...
There exists multiple ways of handle the recognition process, depending on the application. This process can be either a verification, a closed set identification or an open set identification.
...
Verification consists in first enrolling of one or multiple people (references).
...
Unlike a password system where a perfect match is necessary between two strings of characters, biometric feature rarely are identical at different point in time.
This can be due to imperfect sensing conditions (noise in the sensor), alterations in the biometric characteristic (ageing, clothing, make-up), changes in ambient conditions (illumination level), or variations in the user interaction with the system (position or occlusion of the feature).
...
When comparing two feature sets of the same individual, the similarity score is known as *genuine* score. If the score is a result of matching the feature sets of different users, it is known as *impostor* score. 

In order to get the binary answer that we seek, a decision is taken using a threshold value (denoted :math:`\tau`, :math:`\theta`, or :math:`\eta`).
...
False Match (FM)
  ...
  When working with scores: the system gave a high score (higher than the threshold value) to a pair of different identity.
...
False Non-Match (FNM)
  ...
  When working with scores: the system returned a low score (lower than the threshold value) for a pair of matching identity.
...
To evaluate a verification system, a significant number of Genuine pairs and Impostor pairs comparisons must be done, in order to have a good estimate of the performance on the users population.
...
False Match Rate (FMR)
  ...
  It represents the quantity of Impostor pairs of subject and identity that were recognized as *Matches*.
  
  A low FMR is good for high security systems, so less impostors would be authorized wrongly.

False Non-Match Rate (FNMR)
  The FNMR is defined as: :math:`\frac{\Sigma\text{ False Non-Match}}{\Sigma\text{ Condition Positive}} = \frac{\text{FNM}}{\text{FNM}+\text{TM}}`.
  It represents the quantity of Genuine pairs of subject and identity that were recognized as *Non-Matches*.

  A low FNMR increases the user comfort as genuine users won't need to retry multiple times to be recognized by the system.

FMR and FNMR are competing each other since increasing one by changing the threshold value means decreasing the other. So depending on the application, we may choose a threshold value that results in a low FMR or a low FNMR.

Half Total Error Rate
.....................

The Half Total Error Rate (HTER) is an aggregate measure representing both types of error a system commits. It is defined as:
...
An ROC curve presents the True Match Rate (TMR, or 1-FNMR) against the FMR for different threshold values.
...
.. figure:: img/roc_synthetic_comparison.png
  ...
  For every working points, *System C* is performing worse than *System B*, as for each FMR, the corresponding TMR is lower. *System A* is not making any errors.
...
On use, a biometric sample (probe) is compared to every elements of the enrolled set, and the most probable identity is returned.
This is a *one to many* (1:N) operation and is more costly than a verification. The system must compare a given sample to every identity that are registered.
However, the presented sample belongs to one of the enrolled subjects, as it is a closed set of known identities.

This is used for example for searching the identity of a biometric sample in a given database. (fingerprint taken from a crime scene, compared against a list of suspects' fingerprints)

Errors - Rank
^^^^^^^^^^^^^

In order to decide which enrolled identity (model) corresponds to the identity of the probe, the system assigns scores to each comparison made and the highest score is assumed to represent the matching identity.
...
The leftmost point shows how much rank 1 guesses were achieved, the second one cumulates the count of rank 1 and rank 2 guesses, and so on.
...
In order to evaluate systems for biometric recognition, a series of comparisons (verification or identification) must be done on a set of biometric signals originating from a great number of individuals.
To ease the process of researchers and systems designers, datasets of such signals were created. For example, face picture datasets exists were multiple photographs of various people can be used for training or evaluation of face recognition systems.

Multiple datasets exists for each biometric trait, each with different properties:
...
In order to train and evaluate biometric systems, different groups of subjects must be defined. The most common groups that are used are:
...
Biometric data is collected from humans, hence legal requirement for the protection of the identity of experimental subjects are applicable in most jurisdictions.
Most dataset will require the user to sign an agreement on the use of the data, ensuring the privacy of the identity of the subjects.
