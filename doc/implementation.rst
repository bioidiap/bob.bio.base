...
.. note::
    ...
    first check if you could use the :ref:`Verification File List Database <bob.db.bio_filelist>` interface by defining appropriate file lists for the training set, the model set, and the probes.
    Please refer to the documentation :ref:`Documentation <bob.db.bio_filelist>` of this database for more instructions on how to setup this database.
    
    We have implemented the :py:class:`bob.bio.base.database.FileListBioDatabase` to provide an easy interface to use the file lists with ``bob.bio``.
    This wraps the :py:class:`bob.db.bio_filelist.Database` and implements all required functions (see below).
