...
def check_database(
    ...
    if "HOME" in os.environ:
        database_legacy.replace_directories(
            os.path.join(os.environ["HOME"], ".bob_bio_databases.txt")
        )
    ...
    if not skip_train:
        ...
        assert (
            len(
                database_legacy.arrange_by_client(
                    database_legacy.training_files("train_enroller")
                )
            )
            > 0
        )
    ...
    for group in groups:
        model_ids = database_legacy.model_ids_with_protocol(
            group, protocol=protocol
        )
        assert len(model_ids) > 0
        assert (
            database_legacy.client_id_from_model_id(model_ids[0], group)
            is not None
        )
...
def check_database_zt(
    ...
    for group in groups:
        ...
        assert (
            database_legacy.client_id_from_model_id(t_model_ids[0], group)
            is not None
        )
