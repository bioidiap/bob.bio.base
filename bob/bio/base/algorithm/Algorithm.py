    ...
    **Parameters:**
    ...
    def __init__(
        ...
        **kwargs  # parameters from the derived class that should be reported in the __str__() function
        ...
        self.requires_projector_training = (
            performs_projection and requires_projector_training
        )
        self.split_training_features_by_client = (
            split_training_features_by_client
        )
        self.use_projected_features_for_enrollment = (
            performs_projection and use_projected_features_for_enrollment
        )
        self.requires_enroller_training = requires_enroller_training
        self.model_fusion_function = utils.score_fusion_strategy(
            multiple_model_scoring
        )
        self.probe_fusion_function = utils.score_fusion_strategy(
    ...
    def score_for_multiple_models(self, models, probe):
        ...
        **Returns:**
        ...
        if isinstance(models, list):
            return [
                self.probe_fusion_function(self.score(model, probe))
                for model in models
            ]
        elif isinstance(models, numpy.ndarray):
            return [
                self.probe_fusion_function(self.score(models[i, :], probe))
        ...
        **Returns:**
        ...
        if isinstance(probes, list):
            return self.probe_fusion_function(
                [self.score(model, probe) for probe in probes]
            )
        else:
